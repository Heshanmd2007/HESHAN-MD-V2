...
// Patch NodeCache to handle undefined keys
const NodeCache = require('@cacheable/node-cache');
const originalFormatKey = NodeCache.prototype.formatKey;
NodeCache.prototype.formatKey = function(key) {
  if (key === undefined || key === null) {
    console.warn('Undefined cache key detected, using fallback');
    return 'undefined_key_fallback_' + Date.now();
  }
  return originalFormatKey.call(this, key);
};
...
async function connectToWA() {
  ...
  // Clear cache directory before connecting
  try {
    const cacheDir = __dirname + '/auth_info_baileys/cache';
  ...
  conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      const ownerNumber = "94719845166@s.whatsapp.net";
      const upMessage = `
...
`.trim();
  ...
  conn.ev.on('messages.upsert', async({ messages }) => {
    try {
      ...
      const ikratos = ('94719845166', '94719845166', '94719845166');
